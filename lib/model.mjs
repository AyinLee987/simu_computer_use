...
import { readModelConfig, publicModelInfo, codexEnvironment } from './config.mjs';
...
// npm's Windows launcher is a batch file; resolve its known JS entry instead of
// running cmd.exe. Native codex.exe and ordinary POSIX executables run directly.
async function resolveExecutable() {
  const requested = process.env.CODEX_EXECUTABLE || 'codex';
  if (process.platform !== 'win32') return { command: requested, prefix: [] };
  const hasDirectory = requested.includes('/') || requested.includes('\\');
  const folders = hasDirectory ? [''] : (process.env.PATH || '').split(path.delimiter);
  const suffixes = path.extname(requested) ? [''] : ['.exe', '.cmd', '.bat', ''];
  for (const folder of folders) {
    for (const suffix of suffixes) {
      const candidate = hasDirectory ? `${requested}${suffix}` : path.join(folder.replace(/^"|"$/g, ''), `${requested}${suffix}`);
      try { if (!(await stat(candidate)).isFile()) continue; } catch { continue; }
      if (!/\.(cmd|bat)$/i.test(candidate)) return { command: candidate, prefix: [] };
      const entry = path.join(path.dirname(candidate), 'node_modules', '@openai', 'codex', 'bin', 'codex.js');
      try {
        const launcher = await readFile(candidate, 'utf8');
        await access(entry);
        if (launcher.includes('@openai') && launcher.includes('codex')) {
          return { command: process.execPath, prefix: [entry] };
        }
      } catch { /* Try the next installed launcher. */ }
    }
  }
  throw safeError('MODEL_UNAVAILABLE', '未找到可直接启动的 Codex，请检查安装或 CODEX_EXECUTABLE 设置。');
}
...
async function runCodex(args, { prompt = '', signal, timeoutMs = MODEL_TIMEOUT_MS, onEvent } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // stderr may include local paths or account details. Drain it without
    // storing or forwarding it to the HTTP/UI boundary.
...
/** Returns { ok, provider, message }; never exposes login output or credentials. */
export async function checkModelConnection() {
  let config;
  try {
    config = readModelConfig();
    if (config.provider === 'openai') return { ok: true, ...publicModelInfo(config), verified: false, message: 'API 已配置（首次任务时验证连接）' };
    await runCodex(['login', 'status'], { timeoutMs: 15_000 });
    return { ok: true, ...publicModelInfo(config), verified: false, message: '已检测到本机 Codex 登录' };
  } catch (error) {
    return {
      ok: false,
      ...(config ? publicModelInfo(config) : { provider: null }),
      verified: false,
      message: ['MODEL_UNAVAILABLE', 'MODEL_CONFIG'].includes(error.code) ? error.message : 'Codex 尚未就绪，请先在本机完成登录。',
    };
  }
}
